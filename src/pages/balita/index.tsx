...
export default function PosyanduBalita() {
  ...
  useEffect(() => {
    // In a real app, this fetches from Supabase joining peserta and kunjungan_balita
    const fetchKunjungan = async () => {
      try {
        ...
        const { data, error } = await supabase
          ...
          .select(`
            *,
            peserta (nama_lengkap, nik, jenis_kelamin, tanggal_lahir, nama_ibu)
          `)
                ...
                data.map((item, idx) => (
                  <tr key={idx} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900">{item.peserta?.nama_lengkap || 'Tidak diketahui'}</div>
